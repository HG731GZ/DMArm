...
namespace DMArmAPP
{
	public partial class Form1 : Form
	{
		...
		public Form1()
		{
			...
			{
				//避免多次打开同一个子窗体
				form_robot.Close();//关闭窗体
				form_robot.Dispose();//注销窗体线程
			}
			form_robot = new Form_Robot();
			form_robot.StartPosition = FormStartPosition.CenterScreen;//居中显示
			//form_motors.ShowInTaskbar = false;//不在任务栏显示子窗体
			form_robot.Show();
		...
		private void button_start_sending_Click(object sender, EventArgs e)
		{
			if (canfd.IsUpdating == false)
			{
				...
				button_clamp_sw.Enabled = true;
				numericUpDown_clamp_torque.Enabled = true;
				motor_set_zero_enable(false);
			}
			else
			{
				...
				button_enable_motor.Enabled = true;
				button_open_device.Enabled = true;
				button_clamp_sw.Enabled = false;
				numericUpDown_clamp_torque.Enabled = false;
				motor_set_zero_enable(true);
			}
		}
		...
		private void button_clamp_sw_Click(object sender, EventArgs e)
		{
			if ((sender as Button).Text == "夹钳关")
			{
				clamp_close((float)numericUpDown_clamp_torque.Value);
				(sender as Button).Text = "夹钳开";
			}
			else
			{
				clamp_open((float)numericUpDown_clamp_torque.Value);
				(sender as Button).Text = "夹钳关";
			}
		}
		...
		private void button_start_output_Click(object sender, EventArgs e)
		{
			if (!gravity_flag)
			{
				MessageBox.Show("先打开重力补偿！");
				return;
			}
			if (output_flag)
			{
				output_flag = false;
				(sender as Button).Text = "开启力输出";
				trackBar_outputX.Enabled = true;
				trackBar_outputY.Enabled = true;
				trackBar_outputZ.Enabled = true;
				double[] Fh = new double[3];
				rrobot.Fh = new double[3];
			}
			else
			{
				output_flag = true;
				(sender as Button).Text = "关闭力输出";
				trackBar_outputX.Enabled = false;
				trackBar_outputY.Enabled = false;
				trackBar_outputZ.Enabled = false;
				double[] Fh = new double[3];
				Fh[0] = trackBar_outputX.Value;
				Fh[1] = trackBar_outputY.Value;
				Fh[2] = trackBar_outputZ.Value;
				rrobot.Fh = Fh;
			}
		}
		...
		private void numericUpDown_clamp_torque_ValueChanged(object sender, EventArgs e)
		{
			canfd.tools[0].PVT.torque_lim = (float)numericUpDown_clamp_torque.Value;
		...
		private void param_updating_timer_Tick(object sender, EventArgs e)
		{
			...
			label_output_X.Text = "X: " + trackBar_outputX.Value.ToString() + " N";
			label_output_Y.Text = "Y: " + trackBar_outputY.Value.ToString() + " N";
			label_output_Z.Text = "Z: " + trackBar_outputZ.Value.ToString() + " N";
		...
		private void robot_control()
		{
			...
			while (control_running)
			{
				...
				{
					canfd.motors[i].MIT.torque_set = (float)rrobot.Tau_G_Motor[i] * 1.15f;
					if (output_flag)
					{
				...
				if (checkBox_clamp_spring.Checked)
				{
					canfd.tools[0].MIT.torque_set = clamp_spring_torque(canfd.tools[0].Position);
					canfd.tools[0].set();
				}
